...
class FigletFont(object):
    ...
    def preloadFont(cls, font):
        ...
        for extension in ('tlf', 'flf'):
            ...
            # path = '.'#os.path.dirname(__file__)+'/fonts'
            path = 'C:/Users/Gustavo/AppData/Roaming/Sublime Text 2/Packages/ASCIIArt/pyfiglet/fonts'
            try:
                # print('%s/%s' % (path, fn))
                f = open('%s/%s' % (path, fn), 'rb')
                data = f.read()
                f.close()
